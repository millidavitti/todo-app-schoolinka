...
import { ClickEvent, GlobalStateReducer } from "@/utils/interface";
import useReducers from "@/hooks/useReducers";
import CreateTaskName from "./CreateTaskName";
import { formatTime } from "@/utils/helpers";
import CreateTaskDesktop from "./CreateTaskDesktop";

export default function CreateTask({
	globalState,
	globalStateDispatch,
}: GlobalStateReducer) {
	const { taskState, taskStateDispatch } = useReducers();
		...
		<>
			{globalState.creatingTask && (
				<Portal portal='portal'>
					<div
						className='flex flex-col fixed inset-0 bg-[#34405452] justify-end lg:hidden'
						id='overlay'
						onClick={() => {
					...
					>
						{/* Modal */}
						<div
							className=' flex flex-col gap-4 h-[60%] bg-white rounded-xl p-4'
							id='create_task'
